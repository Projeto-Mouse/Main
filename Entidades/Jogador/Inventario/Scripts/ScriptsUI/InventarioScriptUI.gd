...
@onready var grid_inventario = $InventarioUIControl/TextureRect/MarginContainer/GridContainer
...
var slots_do_grid = grid_inventario.get_children()
var colunas_do_grid = grid_inventario.get_columns
...
func pegar_irmoes_cada_slot() -> void:
	for i in range(slots_do_grid.size()):
		...
		if  baixo < slots_do_grid.size():
			slot.focus_neighbor_bottom = slots_do_grid[i + colunas_do_grid].get_path()

		if cima >= 0:
			slot.focus_neighbor_top = slots_do_grid[i - colunas_do_grid].get_path()
